...
class OutputValidator:
    ...
    # National ID patterns (generic, covers many countries)
    NATIONAL_ID_PATTERNS: Final[list[re.Pattern[str]]] = [
        ...
        re.compile(r"\b[A-Za-z0-9]{9,10}\b"),
    ]

    # Bank account patterns
    BANK_ACCOUNT_PATTERN: Final[re.Pattern[str]] = re.compile(
        r"\b\d{8,17}\b"  # Generic account numbers (8-17 digits)
    )
    ROUTING_NUMBER_PATTERN: Final[re.Pattern[str]] = re.compile(
        r"\b\d{9}\b"  # US routing numbers (9 digits)
    )
    ...
    # Address indicators (US-style)
    ADDRESS_INDICATORS: Final[list[str]] = [
        "street",
        "avenue",
        "boulevard",
        "drive",
